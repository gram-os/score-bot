...
def _make_submission(
    session, user_id="u1", game_id="wordle", d=date(2026, 1, 1), rank=2
):
    sub = Submission(
        user_id=user_id,
        username="Tester",
...
class TestFirstSteps:
    def test_awarded_on_first_submission(self, session):
        ...
        streak = _make_streak()
        slugs = check_and_award_achievements(
            session, "u1", "wordle", date(2026, 4, 1), streak, sub, False, 3
        )
    ...
    def test_not_awarded_again(self, session):
        ...
        streak = _make_streak()
        slugs = check_and_award_achievements(
            session, "u1", "wordle", date(2026, 4, 3), streak, sub, False, 3
        )
...
class TestStreakAchievements:
    def test_on_fire_at_seven(self, session):
        ...
        slugs = check_and_award_achievements(
            session, "u1", "wordle", date(2026, 5, 1), streak, sub, False, 3
        )
    ...
    def test_on_fire_not_at_six(self, session):
        ...
        slugs = check_and_award_achievements(
            session, "u1", "wordle", date(2026, 5, 2), streak, sub, False, 3
        )
...
class TestSpeedAchievements:
    def test_speed_demon_on_first_place(self, session):
        ...
        streak = _make_streak()
        slugs = check_and_award_achievements(
            session, "u1", "wordle", date(2026, 6, 1), streak, sub, False, 3
        )
    ...
    def test_speed_demon_not_on_second_place(self, session):
        ...
        streak = _make_streak()
        slugs = check_and_award_achievements(
            session, "u1", "wordle", date(2026, 6, 2), streak, sub, False, 3
        )
...
class TestVarietyAchievements:
    def test_hat_trick_with_three_games(self, session):
        ...
        streak = _make_streak()
        slugs = check_and_award_achievements(
            session, "u1", "connections", today, streak, sub, False, 3
        )
    ...
    def test_completionist_not_with_partial(self, session):
        ...
        streak = _make_streak()
        slugs = check_and_award_achievements(
            session, "u1", "glyph", today, streak, sub, False, 3
        )
...
class TestFreezeAchievement:
    def test_freeze_saver_when_freeze_used(self, session):
        ...
        slugs = check_and_award_achievements(
            session, "u1", "wordle", date(2026, 8, 1), streak, sub, True, 3
        )
    ...
    def test_freeze_saver_not_without_freeze(self, session):
        ...
        streak = _make_streak(current=5)
        slugs = check_and_award_achievements(
            session, "u1", "wordle", date(2026, 8, 2), streak, sub, False, 3
        )
